...
void turnOnDay(int num)
{
  ...
  else
  {
    ...
    else
    {
      ...
      Serial.println("turned ON days till "+ String(num) +", turned OFF others");
    }
  }
}
...
{
  for (int j = 1; j <= 3; j++)//serpentina on then off 3 times
  {  
    for(int i = 0; i < NUM_GIORNI/2; i++) //serpentina on
    {
      digitalWrite(pinsRelaysGiorni[i], LOW);//ON
      digitalWrite(pinsRelaysGiorni[NUM_GIORNI-i-1], LOW);//ON
    ...
    for(int i = NUM_GIORNI/2; i > 0; i--) //serpentina off
    {
      ...
      digitalWrite(pinsRelaysGiorni[NUM_GIORNI-i], HIGH);//OFF
